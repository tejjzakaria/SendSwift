...
if (isset($_POST['submit'])) {
  // Sanitize form data
  $pickupRequestID = mysqli_real_escape_string($conn, $_POST['id']);
  $requestStatus = mysqli_real_escape_string($conn, $_POST['requestStatus']);

  // Update pickup request status in database
  $sql = "UPDATE pickupRequests SET requestStatus='$requestStatus' WHERE id='$pickupRequestID'";
...
$pickupRequestID = mysqli_real_escape_string($conn, $_GET['id']);
...
<body>
  ...
  <div class="row mb-3">
    <label class="col-sm-2 col-form-label" for="requestStatus">Pickup Request Status</label>
    <input type="hidden" name="id" value="<?php echo $pickupRequestData['id']; ?>">
    <div class="col-sm-10">
      <select class="form-select" id="requestStatus" name="requestStatus" required>
        <option value="pending" <?php if ($pickupRequestData['requestStatus'] == 'pending') echo 'selected'; ?>>Pending</option>
        <option value="completed" <?php if ($pickupRequestData['requestStatus'] == 'completed') echo 'selected'; ?>>Completed</option>
      </select>
    </div>
  </div>
  <div class="row justify-content-end">
    <div class="col-sm-10">
      <input type="submit" class="btn btn-primary" name="submit" value="Update Pickup Request Status" />
    </div>
  </div>
</form>
                  </div>
